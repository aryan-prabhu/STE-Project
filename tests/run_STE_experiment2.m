...
for k = 1:nIter
    ...
    estField = arrayfun(@(xx,yy) ...
        ...
        XI, YJ);
    Cth_est = Cth_true;   % true field's absolute threshold, not the estimate's
                           % own peak -- Q_hat no longer cancels out of the
                           % contour, so this now scores position AND strength
                           % jointly instead of position/shape alone

    if ~isfinite(Cth_est) || max(estField(:)) < Cth_est
        % estimated field never reaches the true threshold anywhere --
        % happens when Q_hat is too small; this is a real failure signal
        % under this metric, not a degenerate-input edge case
        warn_hist(k) = true;
    else
